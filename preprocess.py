...
ad_operation = pd.read_table(path + 'ad_operation.dat',
                             names=['aid', 'update_time', 'op_type', 'objective',
                                    'bid_type', 'bid'])
...
for day in tqdm(range(20190410, 20190423), total=len(range(20190410, 20190423))):
    ...
    log_df = pd.read_table(file, names=['aid_request', 'request_time', 'uid',
                                        'aid_location', 'aid_info'])
    print('-' * 50, day, '-' * 50)
...
del logs_item
gc.collect()
logs.columns = ['aid_location', 'day', 'aid', 'filter', 'isExp']
...
# 日志数据增加类别
logs = logs.merge(ad_static_feature[['aid', 'industry_id', 'goods_type', 'goods_id', 'account_id', 'aid_size']],
                  on=['aid'], how='left')
# 测试集样本
atest = pd.read_table(path + 'test_sample_bid.out', names=['sample_id', 'aid', 'objective', 'bid_type', 'bid'])
# 增加类别特征
atest = atest.merge(ad_static_feature[['aid', 'industry_id', 'goods_type', 'goods_id', 'account_id', 'aid_size']],
                    on=['aid'], how='left')
...
# 增加类别特征
btest = btest.merge(ad_static_feature[['aid', 'industry_id', 'goods_type', 'goods_id', 'account_id', 'aid_size']],
                    on=['aid'], how='left')
...
log_df = pd.read_table(path + 'test_tracklog_20190423.last.out',
                       names=['aid_request', 'request_time', 'uid', 'aid_location', 'aid_info'])
items = []
...
#     click_df, train_user, test_user = 1,2,3
#     # 保存预处理文件
